...
export class StudentService {
  ...
  protected convertDateFromClient(student: IStudent): IStudent {
    const copy: IStudent = Object.assign({}, student, {
      dateobirth: student.dateobirth != null && student.dateobirth.isValid() ? student.dateobirth.format(DATE_FORMAT) : null
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateobirth = res.body.dateobirth != null ? moment(res.body.dateobirth) : null;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((student: IStudent) => {
        student.dateobirth = student.dateobirth != null ? moment(student.dateobirth) : null;
      });
    }
    return res;
  }
